...
const VRM0Blendshapes = [
  'joy', 'angry', 'sorrow', 'fun', 'surprised', 'blink', 'blink_l', 'blink_r', 'a', 'i', 'u', 'e', 'o', 'lookup', 'lookdown', 'lookleft', 'lookright'
];

// Start creating the huge dictionary

const dict = {};

// 1. Core Emotions (VRM defaults)
...
visemes.forEach(v => {
  dict[`viseme_${v}`] = { [v]: 1 };
  dict[`viseme_${v}_exaggerated`] = { [v]: 1, jawOpen: 0.5 };
});

// 3. Advanced ARKit Direct Mappings
ARKitBlendshapes.forEach(b => {
   dict[`arkit_${b}`] = { [b]: 1 };
   dict[`arkit_${b}_half`] = { [b]: 0.5 };
   dict[`arkit_${b}_max`] = { [b]: 1.5 };
});

// 4. Action/Anime Tropes
const tropes = {
...
Object.keys(tropes).forEach(k => {
  dict[k] = tropes[k];
});

// 5. Procedural Emotional Plutchik Combinations (8 primary emotions x 8 intensities x 3 variants) = 192 combos
const primaryEmotions = ['joy', 'trust', 'fear', 'surprise', 'sadness', 'disgust', 'anger', 'anticipation'];

// Helper to map Plutchik to VRM/ARKit
const getEmotionMap = (emotion, intensity) => {
  const i = intensity / 10; // 0.1 to 1.0
  switch(emotion) {
...
// 6. Conversational nuances (Combinations of speech parts and micro-expressions)
const nuances = [
  'active_listening', 'polite_smile', 'skeptical_frown', 'thoughtful_pout', 
  'agreeing_nod_smile', 'disagreeing_squint', 'confused_stare', 'realization_gasp',
  'bored_sigh', 'impatient_lip_bite', 'sympathetic_wince', 'awkward_smile',
  'flirtatious_glance', 'sarcastic_smirk', 'deep_concentration'
];
...
nuances.forEach(n => {
   ...
   // Generate intensity variations for nuances too
...
// 7. Micro-expressions (FACS Based)
const FACS = [
  ...
  'AU12_LipCornerPuller', 'AU14_Dimpler', 'AU15_LipCornerDepressor', 'AU17_ChinRaiser',
  'AU20_LipStretcher', 'AU23_LipTightener', 'AU24_LipPresser', 'AU25_LipsPart', 'AU26_JawDrop'
];
...
const FACSMaps = {
  ...
  'AU23_LipTightener': { mouthFunnel: 0.5 }, // Approximation
...
FACS.forEach(au => {
  dict[`FACS_${au}`] = FACSMaps[au];
});

// Compile into a module string
const totalKeys = Object.keys(dict).length;

const outCode = \`import * as THREE from 'three';
import { VRM } from '@pixiv/three-vrm';

// --- Monolith Expression Registry (\${totalKeys} Total Real Mappings) ---
// Generated by Advanced Heuristics Protocol
...
export class NeuralKinematics {
  ...
  public triggerAction(action: string) {
    ...
    // Clear previous non-viseme blendshapes
    if (this.vrm.expressionManager) {
      ...
      // Force clear specific ARKit morphs we use widely so they don't get stuck
  ...
  public triggerNeutral() {
     ...
     // clear all
     const dictKeys = Object.keys(EXPRESSION_DICTIONARY['neutral']);
...
console.log('Successfully wrote ' + totalKeys + ' distinctive expressions to NeuralKinematics.ts');
